...
def nanbyou_system
  ...
  puts "#{@income.to_s(:delimited)}万円ですね"
...
@kougaku_support = @self_pay - self_pay_limit
  ...
  if @kougaku_support < 0
    ...
    puts <<~TEXT
    年収#{@income.to_s(:delimited)}万円の場合、自己負担上限額は#{self_pay_limit.to_s(:delimited)}円です
    残りの#{@kougaku_support.to_s(:delimited)}円も、あなたが加入している医療保険が支払います
    TEXT
  end
end
...
def koureisha_system
  ...
  while true
    ...
    if @income < 0
      ...
      puts "年収#{@income.to_s(:delimited)}万円の場合、自己負担割合は#{old_self_pay_ratio}割です"
    elsif 370 <= @income
      puts "年収#{@income.to_s(:delimited)}万円の場合、自己負担割合は#{old_self_pay_ratio}割です"
    end
    break
